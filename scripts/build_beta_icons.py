#!/usr/bin/env python3
"""Derive unmistakable, size-specific Beta icons from the Stable source art."""
...
def recolor_icon(source: Path) -> Image.Image:
    ...
    for y in range(image.height):
        for x in range(image.width):
            ...
            pixels[x, y] = tuple(
                max(0, min(255, int(value * scale))) for value in BETA_RGB
            ) + (a,)
    return image
...
def write_ico(base: Image.Image, destination: Path) -> None:
    ...
    Pillow's ordinary ICO saver derives requested sizes from one primary image.
    Building the ICONDIR ourselves preserves the separately rendered `B` and
    `BETA` artwork for every Windows icon size.
    """
...
def main() -> int:
    ...
    beta_256.save(BETA_PUBLIC, format="PNG", optimize=False, compress_level=9)
    beta_256.save(BETA_DIR / "icon.png", format="PNG", optimize=False, compress_level=9)
