"""BioLogic MPR header, registration, and cache lifecycle regressions.

Array-level canonical cycle reconstruction is covered separately by
``test_biologic_gcpl``; these tests keep the persisted candidate/promotion and
fail-closed lifecycle boundaries exercised as parser identities evolve.
"""
...
class BiologicClosureIntegrationTests(unittest.TestCase):
    ...
    def test_single_direction_source_registers_canonical_and_builds_cache(self) -> None:
        ...
        self.assertEqual(source.parser_version, "bm:gcpl14cpocv1:r1")
        self.assertEqual(source.row_count, 4)
        self.assertEqual(source.cycle_count, 0)
        ...
        self.assertEqual(cycles["cycle"].tolist(), [])
        raw = cache.load_raw(source.hash, source.parser_version)
        self.assertEqual(raw["cycle"].tolist(), [1, 1, 1, 1])
        self.assertFalse(raw["cycle_complete"].any())
    ...
    def test_biologic_cache_hit_preserves_exact_cycle_identity_provenance(self) -> None:
        ...
        for filename, cycles, cycle_source in cases:
            ...
            index = cache.load_raw_layout_index(file_hash, "bm:gcpl14cpocv1:r1")
    ...
    def test_legacy_gcpl5_source_is_reinspected_under_the_new_cycle_contract(self) -> None:
        ...
        self.assertEqual(refreshed.parser_version, "bm:gcpl14cpocv1:r1")
        self.assertEqual(refreshed.parse_status, "parsed")
        self.assertEqual(refreshed.cycle_count, 0)
        self.assertFalse(parsing.source_record_metadata_only(refreshed))

    def test_gcpl9_source_is_reinspected_after_cycle_identity_revision_bump(self) -> None:
        path = write_gcpl_mpr(
            self.root / "legacy-gcpl9.mpr",
            _single_discharge_rows(),
            settings_payload=_single_discharge_settings(),
            log_payload=encode_gcpl_log(ole_timestamp=45000.0),
            include_log=True,
        ...
        source = scanner.ingest_path(self.db, path, parse_now=True)
        source.parser_version = "bm:gcpl9:r1"
        source.parse_status = "metadata_only"
        source.parse_error = "gcpl9 metadata-only registration"
        source.row_count = None
        source.cycle_count = None
        source.capacity_summary_status = "unavailable"
        self.db.commit()
    ...
    def test_previous_gcpl7_neutral_preamble_registration_is_reinspected_online(self) -> None:
        ...
        self.assertFalse(parsing.source_record_metadata_only(refreshed))
        self.assertTrue(cache.raw_path(refreshed.hash, "bm:gcpl14cpocv1:r1").exists())
        self.assertTrue(
            cache.has_cycles(refreshed.hash, "bm:gcpl14cpocv1:r1", cache.CALC_VERSION)
        )
    ...
    def test_previous_gcpl7_offline_registration_is_fail_closed_and_relinkable(self) -> None:
        ...
        self.assertTrue(old_raw.exists())
        self.assertTrue(old_cycles.exists())
        self.assertFalse(cache.raw_path(refreshed.hash, "bm:gcpl14cpocv1:r1").exists())
        self.assertFalse(
            cache.cycles_path(refreshed.hash, "bm:gcpl14cpocv1:r1", cache.CALC_VERSION).exists()
        )
    ...
    def test_scanner_registers_single_direction_and_builds_a_cache(self) -> None:
        ...
        self.assertEqual(self.source.parser_version, "bm:gcpl14cpocv1:r1")
        self.assertEqual(self.source.row_count, 4)
        self.assertEqual(self.source.cycle_count, 0)
    ...
    def test_gcpl11_identity_is_reinspected_for_capacity_counter_profile(self) -> None:
        self.source.parser_version = "bm:gcpl11:r1"
        self.source.parse_status = "parsed"
        self.db.commit()

        self.assertTrue(parsing.source_uses_legacy_biologic_parser(self.source))
        self.assertEqual(scanner.reinspect_legacy_biologic_sources(self.db), 1)
        self.db.expire_all()

        refreshed = self.db.get(SourceFile, self.source.id)
        self.assertEqual(refreshed.parser_version, "bm:gcpl14cpocv1:r1")
        self.assertEqual(refreshed.parse_status, "parsed")
        self.assertEqual(refreshed.row_count, 4)
    ...
    def test_parser_revision_invalidates_the_previous_canonical_identity(self) -> None:
        ...
        self.assertEqual(current_identity, "bm:gcpl14cpocv1:r1")
    ...
    def test_failed_single_direction_verification_persists_metadata_only_capability(self) -> None:
        ...
        self.assertFalse(
            cache.raw_path(source.hash, "bm:gcpl14cpocv1:r1").exists()
        )
    ...
    def test_previous_gcpl6_identity_is_reinspected_and_offline_rows_fail_closed(self) -> None:
        ...
        self.assertEqual(refreshed_online.parser_version, "bm:gcpl14cpocv1:r1")
