...
def _dcir_profile_started(
    profiling: dict[str, Any] | None,
):
    """Start an opt-in direct compute_dcir boundary without normal-path timing."""
    return perf_counter() if profiling is not None else None


def _dcir_profile_finished(
    profiling: dict[str, Any] | None,
    name: str,
    started: float | None,
) -> None:
...
def compute_dcir(
    ...
    *,
    ...
    dcir_profile_started = _dcir_profile_started(profiling)
    ensure_canonical_cycling_available(
        db,
        spec,
        request_context=request_context,
    ...
    _dcir_profile_finished(profiling, "dcir_compute_setup", dcir_profile_started)
    badges: list[dict] = []
    cell_series: list[dict] = []
    sources_by_cell: dict[int, dict] = {}
    ...
    for series_index, series_cfg in enumerate(configured_series, start=1):
        ...
        if context is None:
            ...
            context_metadata_started = _dcir_profile_started(profiling)
            all_pinned_versions.extend(source_versions[f.hash] for f in files)
            all_current_versions.extend(current_parser_identity(f) for f in files)
            nominal = cell_nominal_capacity_mah(
                cell,
            ...
            _dcir_profile_finished(
                ...
                context_metadata_started,
            )
            for source_file in files:
                cache_hit = False
                reconstructed = None
                protocol_cache_started = _dcir_profile_started(profiling)
                header_meta = source_file.header_meta or {}
                for (
                    cached_header,
                    cached_nominal,
            ...
            target_resolution_started = _dcir_profile_started(profiling)
            ...
            read_plan_started = _dcir_profile_started(profiling)
            step_columns = [
                "record_index",
                "cycle",
                "step",
            ...
            if selected_by_hash:
                ...
                raw, _raw_segments, _missing = stitch.stitch_raw(refs)
            source_context_started = _dcir_profile_started(profiling)
            raw_start = raw.attrs.get("raw_timestamp_start")
            if raw_start is None:
                raw_timestamps = (
                    pd.to_datetime(raw["timestamp"], errors="coerce").dropna()
            ...
            for source_index in required_source_indices:
                source_partition_started = _dcir_profile_started(profiling)
                source_raw = raw.loc[raw["segment"] == source_index].copy()
                _dcir_profile_finished(
                    profiling,
            ...
            context_assembly_started = _dcir_profile_started(profiling)
            context = {
                "hashes": hashes,
                "files": files,
        ...
        series_context_started = _dcir_profile_started(profiling)
        ...
        _dcir_profile_finished(
            ...
            series_context_started,
        )
        for source_index, source_file in enumerate(files):
            target_validation_started = _dcir_profile_started(profiling)
            target = targets_by_source.get(source_index)
            valid_target = bool(target) and not raw.empty
            rest_step = pulse_step = 0
            direction = ""
            prepared = None
            if valid_target:
                try:
            ...
            result_collection_started = _dcir_profile_started(profiling)
        ...
        if occurrence_frames:
            occurrence_concat_started = _dcir_profile_started(profiling)
            ...
            _dcir_profile_finished(
                ...
                occurrence_concat_started,
            )
            relative_started = _dcir_profile_started(profiling)
            ...
            projection_started = _dcir_profile_started(profiling)
            ...
            measurement_meta_started = _dcir_profile_started(profiling)
            measurement_columns = [
                "occurrence",
                "cycle",
                "start_time_h",
            ...
            _dcir_profile_finished(
                ...
                measurement_meta_started,
            )
        else:
            no_match_started = _dcir_profile_started(profiling)
            x_occurrence = []
            x_cycle = []
            x_time = []
            dcir_mohm = []
        ...
        response_assembly_started = _dcir_profile_started(profiling)
    ...
    final_response_started = _dcir_profile_started(profiling)
    for miss in missing_refs:
        badges.append(
            {
                "kind": "missing_reference",
