...
def reconcile_retired_biologic_sources(db: Session) -> int:
    ...
    ``parsing``; the previous gcpl5/gcpl6/gcpl7 identities are handled by the
...
def reinspect_legacy_biologic_sources(db: Session) -> int:
    ...
    support, and gcpl12 capacity-counter interpretation change the canonical
...
def start_capacity_summary_backfill(
    *,
    ...
    try:
        ...
        # state when the current gcpl12 build fails closed.
