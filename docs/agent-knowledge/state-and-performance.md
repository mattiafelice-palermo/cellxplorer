...
cache files, and it includes offline rows. A gcpl3 row becomes current `bm:gcpl7:r1`
metadata-only state, with live cycle and capacity fields cleared. A gcpl4 row becomes current
`bm:gcpl7:r1` only when its stored data header proves the observed 16-ID/53-byte layout; a
