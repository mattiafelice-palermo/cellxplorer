...
cache files, and it includes offline rows. A gcpl3 row becomes current `bm:gcpl12:r1`
metadata-only state, with live cycle and capacity fields cleared. A gcpl4 row becomes current
`bm:gcpl12:r1` only when its stored data header proves a registry-resolved layout; an unrecorded,
