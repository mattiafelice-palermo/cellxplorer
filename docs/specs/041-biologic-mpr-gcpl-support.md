...
**Status:** Plan — implement only after Parent 040 is complete, review-clean, and merged to `main`  
...
**Coordination:** [`041-agent-coordination.md`](041-agent-coordination.md)
...
Users must be able to import a supported BioLogic EC-Lab `.mpr` cycling file and use it as a normal CellXplorer source.
...
Run direct binary-vs-MPT parity, import/cache/source lifecycle, Cycles/Time-Capacity/Steps/DCIR/Rate Capability/Chargeability applicability, performance, existing Neware golden regressions, documentation/context and cumulative review.
...
2. supplied/paired MPR binary fields match EC-Lab `.mpt` ground truth within defined tolerances;
...
13. frozen Windows backend/app can import and reuse a supported MPR when packaged verification is available;
...
The parent is planning-only. Each child owns its implementation record. 041.6 final review summarizes exact 040-based merge base, child/review SHAs, real-file/MPT acceptance evidence, licensing/provenance compliance, performance, packaging/manual status, limitations and merge readiness.
