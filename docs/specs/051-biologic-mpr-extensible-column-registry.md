...
Not yet implemented. Planning amendment: ordinary data-column storage lookup is now defined by the general `encoded_id % 256` base-ID rule while retaining every full encoded ID as source evidence. The registry asset has been updated to the same contract.