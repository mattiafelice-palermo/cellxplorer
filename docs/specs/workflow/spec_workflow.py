...
def cmd_message(args: argparse.Namespace) -> None:
    """Append a clarification message without transferring workflow ownership.

    Messaging is deliberately allowed from either role regardless of TURN so the
    non-owner can answer a clarification while repository-changing ownership stays
    unchanged. The JSON state is never mutated by this command.
    """
    repo = root(); sp, cp, data = resolve(repo, args.spec)
    assert_branch(data)
    if data["action"] in {BLOCKED, COMPLETE}:
        fail(f"Workflow is {data['action']}; clarification messaging is closed.")
    for path in (sp, cp):
        rel = path.relative_to(repo).as_posix()
        if git("status", "--porcelain", "--", rel):
            fail(f"{rel} already has an uncommitted workflow change. Commit/push it before another message.")
    message = (args.message or "").strip()
    if not message:
        fail("message requires --message with the clarification or answer.")
    actor = args.role
    recipient = REVIEWER if actor == IMPLEMENTER else IMPLEMENTER
    ts = now()
    append_entry(
        cp,
        timestamp=ts,
    ...
    print(f"MESSAGE_FROM: {actor}")
    print(f"MESSAGE_TO: {recipient}")
    print(f"ACTIVE_CHILD: {data['active_child']}")
...
def parser() -> argparse.ArgumentParser:
    ...
    x = sub.add_parser("message"); add_spec(x); x.add_argument("--role", required=True, choices=[IMPLEMENTER, REVIEWER]); add_message(x); x.set_defaults(func=cmd_message)
