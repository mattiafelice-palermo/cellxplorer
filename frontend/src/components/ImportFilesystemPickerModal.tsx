    ...
    <Modal opened={opened} onClose={onClose} title="Load cell files" size="68rem">
      <Stack gap="sm">
        ...
        <Group align="stretch" gap="md" wrap="nowrap">
          <Paper p="xs" w={235} style={{ borderRight: "1px solid var(--mantine-color-gray-3)" }}>
            <ScrollArea h={590} type="auto">
              <Stack gap="md">
                      ...
                      {items.map((item) => {
                        ...
                        return (
                          <Group key={`${section}-${item.path}`} gap={4} wrap="nowrap" px="xs" py={6} bg={active ? "var(--mantine-primary-color-0)" : undefined} style={{ borderRadius: 4, opacity: item.available ? 1 : 0.55 }}>
                            <Button variant="subtle" color={active ? "var(--mantine-primary-color-6)" : "dark"} size="compact-sm" leftSection={shortcutIcon(item)} disabled={!item.available} justify="flex-start" style={{ flex: 1, minWidth: 0 }} onClick={() => navigate(item.path)}>
                ...
                <Stack gap={3}>
                  ...
                  {(browseQuery.data?.roots ?? []).map((root) => <Button key={root.path} variant="subtle" color={browseQuery.data?.current_path === root.path ? "var(--mantine-primary-color-6)" : "dark"} size="compact-sm" leftSection={root.name === "Home" ? <IconHome size={15} /> : <IconFolder size={15} />} justify="flex-start" onClick={() => navigate(root.path)}>{root.name}</Button>)}
                </Stack>
              </Stack>
            </ScrollArea>
          </Paper>
          <Stack gap="sm" style={{ flex: 1, minWidth: 0 }}>
            ...
            <Paper withBorder p={0}>
              {browseQuery.isPending ? <Center h={390}><Loader /></Center> : browseQuery.isError ? <Center h={390} px="lg"><Alert color="red" w="100%">{browseQuery.error instanceof Error ? browseQuery.error.message : "This folder could not be opened."}</Alert></Center> : <ScrollArea h={390} type="auto"><Stack gap={0}>
                <Group gap="xs" wrap="nowrap" px="sm" py={8} bg="light-dark(var(--mantine-color-gray-0), var(--mantine-color-dark-6))" style={{ borderBottom: "1px solid var(--mantine-color-gray-3)" }}><Checkbox checked={allVisibleSelected} indeterminate={someVisibleSelected && !allVisibleSelected} readOnly onClick={() => setSelected((current) => { const next = new Map(current); visibleEntries.forEach((entry) => allVisibleSelected ? next.delete(entry.path) : next.set(entry.path, entry)); return next; })} /><Text size="xs" fw={700} style={{ flex: 1 }}>Name</Text><Text size="xs" fw={700} w={90} ta="right">Size</Text><Text size="xs" fw={700} w={145}>Modified</Text></Group>
                {visibleEntries.length === 0 ? <Center h={300}><Text size="sm" c="dimmed">No folders or Neware files here.</Text></Center> : visibleEntries.map((entry) => <Group key={entry.path} gap="xs" wrap="nowrap" px="sm" py={7} bg={selected.has(entry.path) ? "var(--mantine-primary-color-0)" : undefined} style={{ cursor: "pointer", borderBottom: "1px solid var(--mantine-color-gray-2)" }} onClick={(event) => toggleEntry(entry, event.shiftKey, event.ctrlKey || event.metaKey)} onDoubleClick={() => { if (entry.kind === "folder") navigate(entry.path); }}><Checkbox checked={selected.has(entry.path)} readOnly />{entry.kind === "folder" ? <IconFolder size={17} color="var(--mantine-primary-color-6)" /> : <IconFile size={17} color="var(--mantine-color-gray-6)" />}<Text size="sm" truncate style={{ flex: 1 }}>{entry.name}</Text><Text size="xs" c="dimmed" w={90} ta="right">{entry.size === null ? "" : formatBytes(entry.size)}</Text><Text size="xs" c="dimmed" w={145}>{entry.modified_at ? new Date(entry.modified_at).toLocaleString() : ""}</Text>{entry.kind === "folder" && <ActionIcon variant="subtle" color="gray" aria-label={`Open ${entry.name}`} onClick={(event) => { event.stopPropagation(); navigate(entry.path); }}><IconChevronRight size={16} /></ActionIcon>}</Group>)}
              </Stack></ScrollArea>}
            </Paper>
            {selectedEntries.length > 0 && <Paper withBorder p="xs"><Group justify="space-between" mb={4}><Text size="xs" fw={700}>Selected sources</Text><Button size="compact-xs" variant="subtle" color="gray" onClick={() => setSelected(new Map())}>Clear all</Button></Group><ScrollArea h={Math.min(96, selectedEntries.length * 28)} type="auto"><Stack gap={2}>{selectedEntries.map((entry) => <Group key={entry.path} gap="xs" wrap="nowrap">{entry.kind === "folder" ? <IconFolder size={14} /> : <IconFile size={14} />}<Text size="xs" truncate style={{ flex: 1 }}>{entry.path}</Text><ActionIcon size="xs" variant="subtle" color="gray" aria-label={`Remove ${entry.name}`} onClick={() => setSelected((current) => { const next = new Map(current); next.delete(entry.path); return next; })}><IconX size={12} /></ActionIcon></Group>)}</Stack></ScrollArea></Paper>}
          </Stack>
        </Group>
