...
import {
  ...
  type CellPickerCell,
  type CellPickerColumnFilter,
  type CellPickerColumnFilterOperator,
  type CellPickerColumnFilters,
...
import { CellPreviewPlot } from "../../../components/CellPreviewPlot";
import {
...
} from "../../../components/cellPreviewPlotModel";
import {
...
function pickerDateLabel(value: string | null): string {
  if (!value) return "—";
...
type CellPickerColumnWidths = Record<CellPickerSortKey, number>;
...
function AddEntriesModal({
  ...
  const [columnFilters, setColumnFilters] = useState<CellPickerColumnFilters>({});
  ...
  const previewPlotColors = useMemo(() => {
    if (previewSurfaceMode === "theme") return computedColorScheme === "dark"
    ...
    if (previewSurfaceMode === "sun") {
      return { background: theme.white, text: theme.black, border: theme.colors.gray[5], grid: theme.colors.gray[3] };
    }
    if (previewSurfaceMode === "moon") return {
    ...
    return { background: theme.colors.dark[4], text: theme.colors.gray[0], border: theme.colors.dark[1], grid: theme.colors.dark[3] };
  }, [computedColorScheme, previewSurfaceMode, theme]);
  ...
  const cellPickerTableRef = useRef<HTMLTableElement | null>(null);
  const cellPickerColumnRefs = useRef<Record<CellPickerSortKey, HTMLTableColElement | null>>({
    name: null,
    cycle_count: null,
  ...
  const matchesCell = useCallback(
    (cell: CellPickerCell) =>
      ...
      cellMatchesPickerColumnFilters(cell, columnFilters),
    [columnFilters, needle, pickerFilters],
  );
  const sourceFilterOptions = useMemo(() => {
    ...
    const options = (facet: "system" | "format" | "technique") => {
      return cellPickerFacetValues(availableCells, facet, pickerFilters)
        .map((value) => ({ value, label: labelFor(value, facet) }));
    };
    return {
  ...
  const cellPickerTableWidth =
    CELL_PICKER_SELECTION_COLUMN_WIDTH +
    Object.values(cellColumnWidths).reduce((total, width) => total + width, 0);
  const cellSummariesById = useMemo(
    () => new Map((cells.data ?? []).map((cell) => [cell.id, cell] as const)),
    [cells.data],
  );
  const filedReferences = useMemo(() => {
    const cellIds = new Set<number>();
    const groupIds = new Set<number>();
    collectFiledReferences(tree.data?.folders ?? [], cellIds, groupIds);
    return { cellIds, groupIds };
  }, [tree.data]);
  const existingCellIds = useMemo(
    () => new Set(existing.filter((entry) => entry.kind === "cell").map((entry) => entry.ref_id)),
    [existing],
  );
  const filedCellIds = filedReferences.cellIds;
  const filedGroupIds = filedReferences.groupIds;
  const cellRowsByFolderId = useMemo(() => {
    const rowsByFolderId = new Map<number, CellPickerCell[]>();
        ...
        const rows = folder.cells
          .map((cell) => {
            ...
            return {
              id: cell.id,
              name: cell.name,
              cycle_count: summary?.cycle_count_ready ? summary.total_cycles : null,
  ...
  const shownUnfiledCells = hideAddedCells
    ...
    : unfiledCells;
  const visibleFolderCellIds = (folder: FolderNode): number[] => [
    ...(cellRowsByFolderId.get(folder.id) ?? [])
      .filter((cell) => !hideAddedCells || !existingCellIds.has(cell.id))
      .map((cell) => cell.id),
    ...folder.children.flatMap(visibleFolderCellIds),
  ];
  const visiblePickerCellIds = useMemo(() => {
    if (mode !== "cell") return [];
    const visibleIds = (folder: FolderNode): number[] => [
      ...(cellRowsByFolderId.get(folder.id) ?? [])
        .filter((cell) => !hideAddedCells || !existingCellIds.has(cell.id))
        .map((cell) => cell.id),
      ...folder.children.flatMap(visibleIds),
    ];
    return [
      ...visibleFolders.flatMap(visibleIds),
      ...((!branchOnly || !currentFolderId)
        ? shownUnfiledCells.map((cell) => cell.id)
        : []),
    ];
  }, [branchOnly, cellRowsByFolderId, currentFolderId, existingCellIds, hideAddedCells, mode, shownUnfiledCells, visibleFolders]);
  const defaultPreviewCellId = visiblePickerCellIds[0] ?? null;
  useEffect(() => {
    if (previewCellId !== null && !visiblePickerCellIds.includes(previewCellId)) {
      setPreviewCellId(defaultPreviewCellId);
    }
  }, [defaultPreviewCellId, previewCellId, visiblePickerCellIds]);
  const activePreviewCellId = resolveCellPickerPreviewCellId(
    previewCellId,
    defaultPreviewCellId,
    visiblePickerCellIds,
  );
  ...
  const previewSpec = useMemo<AnalysisSpec | null>(() => {
    ...
    return {
      ...
      selection: {
        ...
        entries: [{ kind: "cell", ref_id: activePreviewCellId }],
        exclusions: [],
      ...
      computation: {
        ...
        cycle_range: {
          start: previewCycleCount > 0 ? resolvedPreviewCycleStart : 1,
          end: previewCycleCount > 0 ? resolvedPreviewCycleEnd : 1,
        },
        ...
        time_reference: "selected_range",
          ...
          cycle_end: previewCycleCount > 0 ? resolvedPreviewCycleEnd : 1,
          cycles: previewCycleCount > 0
            ? Array.from({ length: resolvedPreviewCycleEnd - resolvedPreviewCycleStart + 1 }, (_, index) => resolvedPreviewCycleStart + index)
            : [],
          current_right: "none",
          voltage_channel: "voltage",
          voltage_channels: ["voltage"],
          max_points_per_cell: 600,
        },
      },
    };
  }, [activePreviewCellId, analysisSpec, previewCycleCount, previewXAxis, resolvedPreviewCycleEnd, resolvedPreviewCycleStart]);
  const previewQuery = useQuery({
    queryKey: ["analysis-cell-picker-preview", analysisId, activePreviewCellId, previewXAxis, resolvedPreviewCycleStart, resolvedPreviewCycleEnd],
    queryFn: () => post<TimeCapacityResult>(
  ...
  const renderedPreviewXAxis = previewTraceXAxis ?? previewXAxis;
  ...
  const previewChartData = useMemo(() => {
    ...
    const rawX = renderedPreviewXAxis === "time" ? previewTrace.time_s : previewTrace.capacity_mah;
    const x = previewTrace.display_x?.length === previewTrace.cycle.length
      ? previewTrace.display_x
      : rawX.map((value) => value === null ? null : renderedPreviewXAxis === "time" ? value / 60 : value);
    const voltage = previewTrace.voltage_v_by_channel?.voltage ?? previewTrace.voltage_v;
    return cellPreviewVoltageTraces([{
      x,
      voltage,
      current: previewTrace.current_ma,
      voltageColor: PLOT_PALETTES.app[0],
      currentColor: PLOT_PALETTES.app[1],
    }]);
  }, [previewTrace, renderedPreviewXAxis]);
  ...
  const previewCeRange = useMemo(
    () => paddedEfficiencyRange(cyclePreviewSeries?.quantities.coulombic_efficiency_pct ?? []),
    [cyclePreviewSeries],
  );
  const previewCycleChartData = useMemo(() => {
    if (!cyclePreviewSeries) return [];
    const quantities = cyclePreviewSeries.quantities;
    return cellPreviewCapacityTraces([{
      x: cyclePreviewSeries.x,
      chargeCapacityMah: quantities.charge_capacity_mah,
      dischargeCapacityMah: quantities.discharge_capacity_mah,
      efficiencyX: cyclePreviewSeries.x,
      efficiencyPct: quantities.coulombic_efficiency_pct,
      massG: previewActiveMassG,
      chargeColor: PLOT_PALETTES.app[0],
      dischargeColor: PLOT_PALETTES.app[1],
      efficiencyColor: PLOT_PALETTES.app[2],
    }], previewCapacityMode, normalizeCapacityByMass);
  ...
  const previewVoltageLayout = useMemo(
    () => cellPreviewVoltageLayout(
      previewPlotColors,
      renderedPreviewXAxis === "time" ? "Time (min)" : "Capacity (mAh)",
      `cell-picker-voltage-${renderedPreviewXAxis}`,
    ),
    [previewPlotColors, renderedPreviewXAxis],
  );
  const previewCapacityLayout = useMemo(
    () => cellPreviewCapacityLayout(
      previewPlotColors,
      `Capacity (${normalizeCapacityByMass && previewActiveMassG ? "mAh/g" : "mAh"})`,
      previewCeRange,
      `cell-picker-capacity-${normalizeCapacityByMass && previewActiveMassG !== null ? "mAh-per-g" : "mAh"}`,
    ),
  ...
  const preventShiftTextSelection = (event: ReactMouseEvent) => {
    if (event.shiftKey) event.preventDefault();
  };
  const resizeCellColumnBy = (key: CellPickerSortKey, delta: number) => {
    setCellColumnWidths((current) => ({
      ...current,
  ...
  const startCellColumnResize = (key: CellPickerSortKey, event: ReactPointerEvent) => {
    ...
    const startWidth = cellColumnWidths[key];
    const startTableWidth = cellPickerTableWidth;
    let latestX = startX;
    let animationFrame: number | null = null;
    let active = true;
    const widthAt = (clientX: number) =>
      Math.max(minCellPickerColumnWidth(key), startWidth + clientX - startX);
    const applyWidthToDom = (width: number) => {
      const column = cellPickerColumnRefs.current[key];
      if (column) column.style.width = `${width}px`;
      if (cellPickerTableRef.current) {
        cellPickerTableRef.current.style.width =
          `max(100%, ${startTableWidth + width - startWidth}px)`;
      }
    };
    const applyPendingWidth = () => {
      animationFrame = null;
      if (active) applyWidthToDom(widthAt(latestX));
    };
    const removeListeners = () => {
      ...
      if (columnResizeCleanup.current === cancelResize) columnResizeCleanup.current = null;
    };
    const cancelResize = () => {
      if (!active) return;
      active = false;
    ...
    const finishResize = (clientX?: number) => {
      if (!active) return;
      if (clientX !== undefined) latestX = clientX;
      active = false;
      const width = widthAt(latestX);
      applyWidthToDom(width);
      setCellColumnWidths((current) =>
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      if (animationFrame === null) {
        animationFrame = requestAnimationFrame(applyPendingWidth);
      }
    };
    const onPointerUp = (upEvent: PointerEvent) => {
      if (upEvent.pointerId === pointerId) finishResize(upEvent.clientX);
    };
    const onPointerCancel = (cancelEvent: PointerEvent) => {
      if (cancelEvent.pointerId === pointerId) finishResize(cancelEvent.clientX);
    };
    const onWindowBlur = () => finishResize();

    columnResizeCleanup.current = cancelResize;
  ...
  const visibleEntryKeys = [
    ...
    ...((!branchOnly || !currentFolderId)
      ? mode === "cell"
        ? shownUnfiledCells
            .filter((cell) => !has({ kind: "cell", ref_id: cell.id }))
            .map((cell) => keyOf({ kind: "cell", ref_id: cell.id }))
  ...
  const visibleSelectableCellKeys =
    mode === "cell"
      ? Array.from(
          new Set(
            visibleEntryKeys.filter((key) => key.startsWith("cell:") && !has(entryOf(key))),
          ),
        )
      : [];
  const bulkSelectionState = cellPickerBulkSelectionState(visibleSelectableCellKeys, selected);

  const toggleAllVisibleCells = () => {
    if (visibleSelectableCellKeys.length === 0) return;
    const selectVisible = !bulkSelectionState.checked;
    setSelected((current) => toggleCellPickerBulkSelection(current, visibleSelectableCellKeys));
    setLastSelectedKey(
      selectVisible
        ? visibleSelectableCellKeys[visibleSelectableCellKeys.length - 1] ?? null
        : null,
    );
  };
  ...
  const toggleEntry = (
    ...
    event?: Pick<ReactMouseEvent, "shiftKey">,
  ) => {
    if (has(entry)) return;
  ...
  const availableEntryKeys = [
    ...(cells.data ?? []).map((cell) => keyOf({ kind: "cell", ref_id: cell.id })),
    ...(groups.data ?? []).map((group) =>
      keyOf({ kind: "replicate_group", ref_id: group.id }),
    ),
  ];
  const existingEntryKeys = new Set(existing.map(keyOf));
  const selectedEntries = selectableCellPickerSelection(
    selected,
    availableEntryKeys,
    existingEntryKeys,
  ).map(entryOf);

  const defaultColumnFilter = (key: CellPickerSortKey): CellPickerColumnFilter => ({
    operator: key === "name"
      ? "contains"
      : key === "created_at" || key === "last_modified_at"
        ? "before"
        : "eq",
    value: "",
    secondValue: "",
  ...
  const updateColumnFilter = (
    ...
    setColumnFilters((current) => updateCellPickerColumnFilterDraft(
      current,
      key,
      defaultColumnFilter(key),
      patch,
    ));
  };

  const renderColumnFilterControls = (key: CellPickerSortKey, label: string) => {
    const filter = columnFilters[key] ?? defaultColumnFilter(key);
    const isName = key === "name";
    const isNumber = key === "cycle_count" || key === "max_specific_discharge_capacity_mah_g";
    const isDate = key === "created_at" || key === "last_modified_at";
    const operatorOptions: { value: CellPickerColumnFilterOperator; label: string }[] = isName
    ...
    return (
      <Stack gap="xs" w={270}>
        ...
        <Divider />
        <Text size="xs" fw={700}>Filter {label.toLowerCase()}</Text>
        <Select
          size="xs"
          aria-label={`${label} filter condition`}
          value={filter.operator}
          data={operatorOptions}
          onChange={(operator) => updateColumnFilter(key, { operator: operator ?? defaultColumnFilter(key).operator })}
        ...
        {isName ? (
          <TextInput
            ...
            value={filter.value}
            onChange={(event) => updateColumnFilter(key, { value: event.currentTarget.value })}
          />
        ) : isNumber ? (
          <Group grow gap={6}>
            <NumberInput
              size="xs"
              aria-label={`${label} filter ${filter.operator === "between" ? "minimum" : "value"}`}
              placeholder={filter.operator === "between" ? "Minimum" : "Value"}
              value={filter.value}
              onChange={(value) => updateColumnFilter(key, { value: String(value ?? "") })}
              hideControls
            />
            {filter.operator === "between" && (
              <NumberInput
                ...
                onChange={(value) => updateColumnFilter(key, { secondValue: String(value ?? "") })}
                hideControls
              />
            )}
          </Group>
          ...
          <Group grow gap={6}>
            <TextInput
              size="xs"
              type="date"
              aria-label={`${label} filter ${filter.operator === "between" ? "start date" : "date"}`}
              value={filter.value}
              onChange={(event) => updateColumnFilter(key, { value: event.currentTarget.value })}
            />
            {filter.operator === "between" && (
              <TextInput
                size="xs"
                type="date"
                aria-label={`${label} filter end date`}
                value={filter.secondValue}
                onChange={(event) => updateColumnFilter(key, { secondValue: event.currentTarget.value })}
              />
            )}
          </Group>
        ) : null}
        {cellPickerColumnHasFilter(columnFilters[key], key) && (
          <Button size="xs" variant="subtle" onClick={() => updateColumnFilter(key, { value: "", secondValue: "" })}>
            Clear this filter
          </Button>
        )}
      </Stack>
    ...
    return (
      ...
      >
        <Group wrap="nowrap" gap={2} justify="space-between" pr={12}>
          ...
          >
            <Popover.Target>
              <Tooltip label={`Sort and filter ${label}`}>
                <ActionIcon
                  size="sm"
                  variant={cellPickerColumnHasFilter(columnFilters[key], key) ? "filled" : "subtle"}
                  color={cellPickerColumnHasFilter(columnFilters[key], key) ? "primary" : "gray"}
                  aria-label={`Sort and filter ${label}`}
                  aria-expanded={openColumnFilter === key}
                  onClick={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
            ...
            <Popover.Dropdown p="sm">
              {renderColumnFilterControls(key, label)}
            </Popover.Dropdown>
          </Popover>
        </Group>
        <Box
          ...
          tabIndex={0}
          onPointerDown={(event) => startCellColumnResize(key, event)}
          onKeyDown={(event) => {
            if (event.key !== "ArrowLeft" && event.key !== "ArrowRight") return;
            event.preventDefault();
            event.stopPropagation();
            resizeCellColumnBy(key, event.key === "ArrowRight" ? 16 : -16);
          }}
          style={{
            position: "absolute",
            top: 0,
  ...
  const renderFolderRows = (folder: FolderNode, depth: number): ReactNode[] => {
    ...
    const visibleFolderCells = (cellRowsByFolderId.get(folder.id) ?? []).filter(
      (cell) => !hideAddedCells || !existingCellIds.has(cell.id),
    );
    ...
    const rows: ReactNode[] = [
      <Table.Tr key={`folder-${folder.id}`} style={{ cursor: entries.length ? "pointer" : "default" }}>
        ...
        <Table.Td colSpan={mode === "cell" ? 5 : 1}>
          ...
          >
            <IconFolder size={14} color="var(--mantine-primary-color-6)" style={{ flexShrink: 0 }} />
            <Text
              size="xs"
              fw={700}
              c="dimmed"
              truncate
              title={folder.name}
              style={{ minWidth: 0, flex: "0 1 auto", whiteSpace: "nowrap" }}
            >
              {folder.name}
            </Text>
            {entries.length > 0 && (
              <Badge size="xs" variant="light" style={{ flex: "0 0 auto" }}>
                {entries.length}
              </Badge>
            )}
          </Group>
    ...
    if (mode === "cell") {
      visibleFolderCells.forEach((cell) => {
          ...
          rows.push(
            <Table.Tr
              ...
              bg={activePreviewCellId === cell.id
                ? "light-dark(var(--mantine-primary-color-0), var(--mantine-color-dark-5))"
                : selected.has(key) ? "light-dark(var(--mantine-primary-color-0), var(--mantine-primary-color-9))" : undefined}
              aria-selected={activePreviewCellId === cell.id}
              tabIndex={0}
              style={{
                cursor: "pointer",
                boxShadow: activePreviewCellId === cell.id
                  ? "inset 3px 0 var(--mantine-primary-color-6)"
                  : undefined,
              }}
              onMouseDown={preventShiftTextSelection}
              onClick={(event) => {
                setPreviewCellId(cell.id);
                if (event.shiftKey) toggleEntry(entry, event);
              }}
              onKeyDown={(event) => {
            ...
            >
              <Table.Td w={42}>
                <Checkbox
                  checked={added || selected.has(key)}
                  disabled={added}
                  aria-label={`${selected.has(key) ? "Remove" : "Add"} ${cell.name} ${selected.has(key) ? "from" : "to"} selection`}
              ...
              <Table.Td>
                <Group
                  ...
                  style={{ minWidth: 0, width: "100%", overflow: "hidden" }}
                >
                  <IconDatabase size={14} color={selected.has(key) || activePreviewCellId === cell.id ? "light-dark(var(--mantine-color-gray-7), var(--mantine-color-gray-1))" : "var(--mantine-color-gray-6)"} style={{ flexShrink: 0 }} />
                  <Text
                    size="sm"
                    fw={600}
                    c={selected.has(key) || activePreviewCellId === cell.id ? CELL_PICKER_SELECTED_TEXT_COLOR : undefined}
                    truncate
                    title={cell.name}
                  ...
                  {activePreviewCellId === cell.id && (
                    <Badge size="xs" variant="light" color="primary" style={{ flex: "0 0 auto" }}>
                      Preview
                    </Badge>
                  )}
                  {added && (
      ...
      <Stack gap="sm" style={{ height: "100%", minHeight: 0 }}>
          ...
          {mode === "cell" && (
            <Group gap="xs" wrap="nowrap" style={{ flex: "0 0 auto" }}>
                  ...
                  <Stack gap="sm">
                    <Group grow align="flex-start" wrap="nowrap">
                      <MultiSelect
                        ...
                        onChange={(sourceSystems) => setPickerFilters((current) =>
                          updateCellPickerFacetFilters(cells.data ?? [], current, "system", sourceSystems))}
                        searchable
                        clearable
                      ...
                      <MultiSelect
                        ...
                        onChange={(fileFormats) => setPickerFilters((current) =>
                          updateCellPickerFacetFilters(cells.data ?? [], current, "format", fileFormats))}
                        searchable
                        clearable
                        hidePickedOptions={false}
                        maxDropdownHeight={240}
                      ...
                      <MultiSelect
                        ...
                        onChange={(techniques) => setPickerFilters((current) =>
                          updateCellPickerFacetFilters(cells.data ?? [], current, "technique", techniques))}
                        searchable
                        clearable
                        hidePickedOptions={false}
                        maxDropdownHeight={240}
              ...
              <Button
                ...
                aria-label={
                  bulkSelectionState.checked
                    ? `Deselect all ${visibleSelectableCellKeys.length} shown cells`
                    : `Select all ${visibleSelectableCellKeys.length} shown cells (${bulkSelectionState.selectedCount} currently selected)`
                }
                disabled={visibleSelectableCellKeys.length === 0}
                onClick={toggleAllVisibleCells}
              >
                {bulkSelectionState.checked ? "Deselect shown" : "Select all shown"}
              </Button>
              <Button
                size="sm"
                variant={hideAddedCells ? "light" : "default"}
                leftSection={hideAddedCells ? <IconEye size={16} /> : <IconEyeOff size={16} />}
                aria-pressed={hideAddedCells}
                disabled={existingCellIds.size === 0}
                onClick={() => setHideAddedCells((current) => !current)}
              >
                {hideAddedCells ? "Show cells in plot" : "Hide cells in plot"}
              </Button>
            </Group>
          )}
        </Group>
        ...
        <ScrollArea type="auto" offsetScrollbars scrollbarSize={10} style={{ height: "100%", minHeight: 0 }}>
          <Table
            ...
            highlightOnHover
            style={{
              width: mode === "cell" ? `max(100%, ${cellPickerTableWidth}px)` : undefined,
              minWidth: mode === "cell" ? "100%" : undefined,
              tableLayout: mode === "cell" ? "fixed" : undefined,
              userSelect: "none",
          ...
          >
            {mode === "cell" && (
              <>
                <colgroup>
                  <col style={{ width: CELL_PICKER_SELECTION_COLUMN_WIDTH }} />
                  <col
                    ref={(node) => {
                      cellPickerColumnRefs.current.name = node;
                    }}
                    style={{ width: cellColumnWidths.name }}
                  />
                  <col
                    ref={(node) => {
                      cellPickerColumnRefs.current.cycle_count = node;
                    }}
                    style={{ width: cellColumnWidths.cycle_count }}
                  />
                  <col
                    ref={(node) => {
                      cellPickerColumnRefs.current.max_specific_discharge_capacity_mah_g = node;
                    }}
                    style={{ width: cellColumnWidths.max_specific_discharge_capacity_mah_g }}
                  />
                  <col
                    ref={(node) => {
                      cellPickerColumnRefs.current.created_at = node;
                    }}
                ...
                <Table.Thead>
                  <Table.Tr>
                    <Table.Th
                      w={CELL_PICKER_SELECTION_COLUMN_WIDTH}
                      style={{
                        position: "sticky",
                        top: 0,
                        zIndex: 5,
                    ...
                    />
                    {renderCellSortHeader("name", "Cell")}
            ...
            <Stack gap="xs" style={{ minHeight: "100%" }}>
              <div>
                ...
                <Text size="xs" c="dimmed" truncate title={previewCellSummary?.name ?? ""}>
                  {previewCellSummary?.name ?? "Select a cell to preview"}
                </Text>
              </div>
                ...
                cyclePreviewQuery.isLoading && !cyclePreviewSeries ? (
                  ...
                  <>
                    <CellPreviewPlot
                      ...
                      updating={cyclePreviewQuery.isFetching && Boolean(cyclePreviewSeries)}
                    />
                  </>
              ...
              {previewQuery.isLoading && !previewTrace ? (
                ...
                <>
                  <CellPreviewPlot
                    ...
                    surfaceMode={previewSurfaceMode}
                    onSurfaceModeChange={setPreviewSurfaceMode}
                    updating={previewQuery.isFetching && Boolean(previewTrace)}
                    legend={[{ name: "Voltage", color: PLOT_PALETTES.app[0] }, { name: "Current", color: PLOT_PALETTES.app[1] }]}
                  />
                </>
              ) : (
