...
import { clearAnalysisQueryCache } from "../workspace/analysisQueryCache";
import { AnalysisDatabaseTable } from "./AnalysisDatabaseTable";
import {
...
export type AnalysesIndexRouteIntent = {
  openCreate: boolean;
  openPortableImport: boolean;
  portableSource: string | null;
};

export type AnalysesIndexNavigationOptions = {
  plotId?: string;
};

export type AnalysesIndexViewProps = {
  routeIntent: AnalysesIndexRouteIntent;
  consumeRouteKeys: (keys: Array<"new" | "portableImport" | "portableSource">) => void;
  navigateToAnalysis: (analysisId: number, options?: AnalysesIndexNavigationOptions) => void;
...
export function AnalysesIndexView({
  routeIntent,
  consumeRouteKeys,
  navigateToAnalysis,
  navigateToFolder,
}: AnalysesIndexViewProps) {
  ...
  const [portableImportOpen, setPortableImportOpen] = useState(routeIntent.openPortableImport);
  ...
  useEffect(() => {
    if (routeIntent.openPortableImport) setPortableImportOpen(true);
  }, [routeIntent.openPortableImport]);
  ...
  const resetPortable = (discard = true) => {
    ...
    consumeRouteKeys(["portableImport", "portableSource"]);
    if (discard && token) {
  ...
  const portableSource = routeIntent.portableSource;
  useEffect(() => {
    if (
      !portableSource ||
    ...
    if (portableReview) {
      ...
      inspectPortable.reset();
      inspectPortablePath.reset();
      if (token) del(`/api/analyses/portable-import-staged/${token}`).catch(() => undefined);
    }
    setPortableImportOpen(true);
    handledPortableSource.current = portableSource;
  ...
  const importPortable = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      navigateToAnalysis(result.analysis.id);
    },
    onError: (error: Error) =>
      notifications.show({ message: error.message, color: "red" }),
  });
  ...
  useEffect(() => {
    ...
    openCreateAnalysis();
    consumeRouteKeys(["new"]);
  }, [consumeRouteKeys, openCreateAnalysis, routeIntent.openCreate]);
  ...
  return (
    ...
    if (!background) {
      ...
      window.requestAnimationFrame(() => navigateToAnalysis(analysis.id));
            }
          }}
          onOpenPlot={(analysis, plot, background) => {
            ...
            if (!background) {
              ...
              window.requestAnimationFrame(() =>
                navigateToAnalysis(analysis.id, { plotId: plot.id }),
              );
            }
          }}
